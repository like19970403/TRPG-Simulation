...
## 技術棧重新評估紀錄（Technology Re-evaluation Notes）

### 2026-02-28：Gorm / Gin / 替代方案評估

**觸發原因：** 專案 v0.10.0 完成（29 條路由、5,305 行生產代碼、477 個測試），評估是否替換現有 pgx/v5 + net/http。

**評估範圍：** Gorm、Gin、sqlc、Bun、ent、sqlboiler、chi

**結論：維持現有技術棧。**

| 技術 | 決定 | 理由 |
|------|------|------|
| Gin | 不採用 | 本專案已有 `writeJSON`/`writeError`/`UserClaimsFromContext` 等 helper，Gin 可讀性收益 ≈ 0；`gin.Context` 型別安全反而較差 |
| chi | 未來考慮 | handler 簽名與 net/http 完全相容，遷移零成本；路由達 40-50 條時引入 route grouping |
| Gorm | 不採用 | JSONB 處理需額外 datatypes 套件、Event Sourcing 與 mutation hooks 衝突、複雜查詢仍需 `db.Raw()` |
| sqlc | 未來可漸進採用 | SQL-first 代碼生成，減少 `row.Scan` 樣板、編譯期型別檢查；可與 pgx 共存 |
| Bun / ent / sqlboiler | 不採用 | 對本專案 JSONB + Event Sourcing + 複雜 SQL 的組合無明顯優勢 |

**安全性審計：** 35+ 個資料庫方法全部使用 pgx 參數化查詢（`$1, $2`），無字串拼接用戶輸入，無動態 WHERE/LIKE。Raw SQL 在開源專案中不構成額外安全風險。

**下次重新評估觸發點：**
- HTTP 路由數達 40-50 條 → 評估引入 chi
- 新增 2-3 個新 repository → 評估用 sqlc 撰寫新代碼
