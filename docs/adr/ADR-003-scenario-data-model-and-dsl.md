...
scenario:
  ...
  items:
    - id: "rusty_key"
      ...
      gm_notes: "通往地下室密道的鑰匙，可考慮在圖書館發現後給予。"  # 可選，僅 GM 可見
    ...
    - id: "healing_potion"
      name: "治療藥水"
      type: "item"
      description: "散發淡淡草藥香味的玻璃瓶。"
      stackable: true  # 可選，預設 false。true 時可堆疊，數量可累加

    - id: "crystal_ball"
      name: "水晶球"
      type: "prop"
      description: "一顆散發微弱紫光的水晶球。"
      # image、gm_notes、stackable 欄位可省略
  ...
  scenes:
    - id: "entrance"
      ...
      on_enter:
        - set_var: { name: "ghost_anger", expr: "var('ghost_anger') + 1" }  # expr: 表達式求值
        # 也可用字面值：set_var: { name: "visited", value: true }
    ...
    - id: "library_discovery"
      ...
      on_enter:
        - give_item: { item_id: "torn_diary", to: "current_player" }        # 給予道具到背包
        - give_item: { item_id: "healing_potion", to: "all", quantity: 2 }  # 給予全體各 2 瓶
        - set_var: { name: "found_secret_passage", value: true }
        # 其他可用 action: reveal_item（legacy）、remove_item、reveal_npc_field
...
├── Items[]              (id, name, type, description, gm_notes?, image?, stackable?)
├── NPCs[]               (id, name, image?, fields[])
│   └── Field            (key, label, value, visibility: public/hidden/gm_only)
├── Scenes[]
│   ├── id, name, content, gm_notes
│   ├── items_available[] (引用 Items.id — GM 參考用，提示此場景可發放的道具)
│   ├── npcs_present[]    (引用 NPCs.id)
│   ├── on_enter[]        (Action: set_var / give_item / remove_item / reveal_item / reveal_npc_field)
│   ├── on_exit[]         (Action)
│   └── Transitions[]
│       ├── target        (引用 Scenes.id)
...
| 函式 | 簽名 | 說明 |
|------|------|------|
| `has_item` | `has_item(item_id: string) → bool` | 當前玩家是否持有指定道具（檢查 PlayerInventory） |
| `item_count` | `item_count(item_id: string) → int` | 當前玩家持有指定道具的數量 |
| `all_have_item` | `all_have_item(item_id: string) → bool` | 所有線上玩家都持有指定道具 |
| `roll` | `roll(notation: string) → int` | 擲骰並回傳結果（如 `roll('2d6')`） |
| `attr` | `attr(name: string) → int` | 讀取當前玩家的角色屬性值 |
| `var` | `var(name: string) → any` | 讀取劇本變數值 |
| `player_count` | `player_count() → int` | 當前遊戲線上玩家人數 |
...
- **`hidden`**：GM 在遊戲中手動揭露給指定玩家（或由 on_enter action 揭露）
- **`gm_only`**：僅 GM 可見，永遠不揭露給玩家（如 GM 演出提示、劇情線索）
...
| 觸發類型 | 評估時機 | 說明 |
|----------|---------|------|
| `gm_decision` | GM 手動選擇 | GM 在控制台點擊切換場景 |
| `player_choice` | 玩家選擇 | 玩家看到符合條件的選項並投票/選擇 |
| `condition_met` | 場景載入 + 狀態變更時 | 條件為 true 時自動觸發轉場 |
| `auto` | 場景載入後立即 | 無條件自動跳轉（用於中間場景） |

**condition_met 評估時機：**
- 場景切換完成後
- `variable_changed`、`item_given`、`item_removed` 事件後，Server 重算轉場條件並推送 `transitions_updated` 事件

**auto 鏈式跳轉：**
- auto 觸發器會立即執行目標場景的 on_enter → 檢查是否還有 auto 觸發器 → 繼續跳轉
- **最大鏈式深度：10**（`maxTransitionChainDepth`），超過則中止並 log warning
- 防止無限迴圈（如 A → auto → B → auto → A）
...
4. **表達式驗證**：
   ...
   - 引用的函式存在（`has_item`、`item_count`、`all_have_item`、`roll`、`attr`、`var`、`player_count`）
5. **限制檢查**：
   - 場景數量 ≤ 200
...
**後續追蹤：**
- [x] SPEC-017：sample scenario + format guide（已完成）
- [x] SPEC-008：expr 自訂函式實作（已完成，7 個函式）
- [x] SPEC-019：視覺化劇本編輯器（已完成）
- [ ] 未來考慮：極複雜劇本疊加 Lua 層
